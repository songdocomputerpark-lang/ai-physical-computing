...
2. **눈으로 확인:** 사진·그림(png·jpg 등, 사진이 든 SVG 포함)은 한 장씩 열어 얼굴·이름·화면 속 경로·파일 이름·기기 주소·학교명이 없는지 보고 `scripts/image-allowlist.yaml`에 기록해요. 기록이 없으면 커밋이 막혀요.
...
2. 기록의 한국어 메시지로 원인을 찾아요.
   ...
   | `[저장소 검사] 실패` | 원본 형식·5MB 초과·원본 파일 이름·개인정보 모양·확인 기록 없는 그림이 있어요 | 메시지 아래 "고치는 법"을 따라요 |
...
- `npm run build`의 마지막 단계에서 Pagefind가 `dist/pagefind/`에 검색 색인을 만들어요.
- 색인에 넣는 곳은 페이지 본문(`<main>`)이에요. 검색 페이지와 404 페이지는 빼요.
