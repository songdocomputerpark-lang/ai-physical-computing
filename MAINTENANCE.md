...
| `pages` | 교과서·읽기·마무리는 꼭 | 교과서 쪽. 예: `"008~012"`. 원고가 없는 차시는 `"파일명 p55·p58"` |
| `standards` | 꼭(비어도 적기) | 성취기준 코드 목록 — **PLAN §2.2 대응표(=`src/config/standards.ts`)와 똑같이**. 대응이 없는 차시는 `[]` → 화면은 "성취기준 코드 확인 중"(DECISIONS C8). 코드를 지어내지 않아요 |
...
- 원고 속 얼굴이 나온 사진, 학생 이름, 화면 속 경로·기기 주소는 쓰지 않아요(3-1절). 얼굴로 사람을 알아보는 활동도 넣지 않아요(PLAN §10).
...
- 원고 그림(래스터)은 **원고 이미지 추출 도구로만** 꺼내요 — 목록 → 꺼내기 → 한 장씩 눈 확인 → 대체 글(3-1절). 본문에는 `![목록의 alt 그대로](/images/lessons/1-1-1/third-party/publisher/recognition.webp)`. 검사가 목록의 alt와 본문 대체 글이 같은지, 눈 확인 기록(`reviewed`)이 있는지 봐요.
...
접기 **맨 끝에는 "이 차시의 원고와 자료"가 저절로 붙어요**(`src/components/lesson/LessonTeacherInfo.astro`) — 원천(원고를 옮겼는지 / "원고 없음: 사이트가 코드 기준으로 쓴 본문" / 보충), 성취기준(코드·영역·사이트 요약·"차시 연결은 사이트가 붙임"·교육과정 원문 게시물 링크, 비었으면 "성취기준 코드 확인 중"), 원본 파일 안내(원본 내려받기는 없음 — 개인정보 때문, 예제는 실습실·갤러리에서), 가린 편집본 교안(`handouts`에 적은 쪽 — 편집본 PDF가 교사용 자료실에 올라오면 그 쪽으로 가는 링크가 저절로 생겨요, `src/components/lesson/handouts.ts`). 차시마다 손으로 적지 않아요.
