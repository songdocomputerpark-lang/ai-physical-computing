...
// P2-02부터 코드 입력칸은 CodeMirror 에디터(실습실 공통 틀 LabShell)다 — 도구는 helpers/lab.ts.
import { expect, test, type Page } from '@playwright/test';
import { ALLOWED_REMOTE_ORIGINS, PYODIDE_VERSION, STOP_GRACE_MS } from '../../src/lab/runtime/config.ts';
import { DEV_LAB_PATH, LOAD_TIMEOUT, labRoot, openLabAndWaitReady, runCode, waitDone } from './helpers/lab.ts';
...
test.describe('파이썬 실행기(개발용 시험 페이지)', () => {
  ...
  test('Pyodide를 받아 준비되고, JSPI 감지 결과를 보이며, print("안녕")이 콘솔에 나오고 오류는 트레이스백과 함께 보인다', async ({ page }) => {
    ...
    const box = labRoot(page);
    await expect(box).toHaveAttribute('data-jspi', 'yes');
    await expect(box).toHaveAttribute('data-limited', 'no');
    await expect(page.locator('[data-dev-jspi]')).toContainText('워커 쪽(can_run_sync): 있음');
    await expect(page.locator('[data-dev-engine]')).toContainText(`Pyodide ${PYODIDE_VERSION}`);
    await expect(page.locator('[data-dev-engine]')).toContainText('Python 3.14');
    await expect(page.locator('[data-lab-limited]')).toBeHidden();
    ...
    await expect(page.locator('[data-lab-result]')).toContainText("NameError: name 'undefined_name' is not defined");
    await expect(consoleBox).toContainText('File "main.py", line 1');

    await runCode(page, 'print(\n');
    expect(await waitDone(page)).toBe('error');
    await expect(page.locator('[data-lab-result]')).toContainText('SyntaxError');
    ...
    expect(await waitDone(page)).toBe('ok');
    await expect(page.locator('[data-lab-result]')).toHaveText('exit()로 끝났어요(종료 코드 없음).');
  ...
  test('정지 1단계: time.sleep으로 기다리는 반복문이 [정지] 뒤 1초 안에 KeyboardInterrupt로 멈추고 다시 실행할 수 있다', async ({ page }) => {
    ...
    await expect(page.locator('[data-lab-console]')).toContainText('돌고 있어요 3', { timeout: 10_000 });
    ...
    expect(await waitDone(page)).toBe('ok');
    await expect(page.locator('[data-lab-console]')).toContainText('다시');
  });
  ...
  test('정지 2단계: 계산만 하는 반복문은 파이썬을 다시 시작해서 멈추고, 그 뒤 다시 실행할 수 있다', async ({ page }) => {
    ...
    expect(await waitDone(page)).toBe('ok');
    await expect(page.locator('[data-lab-console]')).toContainText('재시작 뒤');
  });
  ...
  test('대기 지점: input()이 입력줄을 띄우고 화면이 넣은 값을 받으며, 기다리는 동안 [정지]하면 바로 멈춘다', async ({ page }) => {
    ...
    await page.locator('[data-lab-input]').fill('민수');
    await page.keyboard.press('Enter');
    expect(await waitDone(page)).toBe('ok');
    await expect(page.locator('[data-lab-console]')).toContainText('안녕, 민수');
    ...
    await page.getByRole('button', { name: '정지', exact: true }).click();
    expect(await waitDone(page, 5_000)).toBe('stopped');
    expect(Number(await labRoot(page).getAttribute('data-stop-ms'))).toBeLessThan(STOP_GRACE_MS);
    await expect(form).toBeHidden();
  });
  ...
  test('제한 모드(?limited=1): 안내가 보이고, 한 번 실행되는 코드는 돌지만 input()은 한국어 설명과 함께 오류가 난다', async ({ page }) => {
    ...
    await expect(page.locator('[data-lab-limited]')).toBeVisible();
    await expect(page.locator('[data-lab-console]')).toContainText('제한 모드로 실행해요');

    await runCode(page, 'print(1 + 1)\n');
    expect(await waitDone(page)).toBe('ok');
    await expect(page.locator('[data-lab-console]')).toContainText('2');

    await runCode(page, "input('x')\n");
    expect(await waitDone(page)).toBe('error');
    await expect(page.locator('[data-lab-result]')).toContainText('RuntimeError');
    await expect(page.locator('[data-lab-result]')).toContainText('JSPI');
  });
  ...
  test('패키지: import numpy를 쓰면 Pyodide 패키지를 받아 실행되고, 진행 상황이 보인다', async ({ page }) => {
    ...
    await expect(page.locator('[data-lab-console]')).toContainText('3');
    const pageOrigin = new URL(page.url()).origin;
    for (const origin of origins) {
      expect([pageOrigin, ...ALLOWED_REMOTE_ORIGINS], origin).toContain(origin);
    }
...
test.describe('좁은 화면', () => {
  ...
  test('시험 페이지가 화면보다 넓어지지 않고 실습실 칸이 세로로 쌓인다', async ({ page }) => {
    ...
    // 편집칸 → 입력과 출력 → 조절 패널 → 콘솔 순서로 위에서 아래로 놓인다.
    const tops = await page.evaluate(() =>
      ['.lab__editor', '.lab__io', '.lab__panel', '.lab__console'].map((selector) => document.querySelector(selector)?.getBoundingClientRect().top ?? -1),
    );
