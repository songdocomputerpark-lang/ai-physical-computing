...
import { BRIDGE_MIN_INTERVAL_MS, BridgeOutbox, rawMessage, textMessage, type BridgeMessage, type BridgeWarning } from '../../../src/lab/bridge/index.ts';
...
describe('§7.2 규칙 5 — 상태는 최신 값, 이벤트는 대기열에 보존', () => {
  ...
  it('상태와 이벤트가 섞여도 순서는 그대로고 상태만 바뀐다', async () => {
    const clock = new FakeScheduler();
    const sink = immediate();
    const outbox = new BridgeOutbox(sink.write, { scheduler: clock });
    ...
    outbox.send(textMessage('2,2')); // 앞의 1,1 자리를 바꾼다(이벤트를 건너뛰지 않는다)
    expect(outbox.pending.map((message) => message.text)).toEqual(['2,2', 'DATA,9,9,1,0']);
  });
});
...
describe('§7.6 — 원본 코드용 병합 규칙', () => {
  ...
  it('② 머리말·필드 수가 같은 메시지는 그 자리에서 바뀐다', () => {
    const clock = new FakeScheduler();
    const sink = immediate();
    const outbox = new BridgeOutbox(sink.write, { scheduler: clock });
  ...
  it('③ DATA 5필드에서 클릭 표시가 1이면 바뀌지 않는다', async () => {
    ...
    outbox.send(rawMessage(bytes('DATA,1931,1076,0,0'))); // 다시 보통 — 앞의 보통 프레임 자리를 바꾼다

    expect(outbox.pending.map((message) => message.text)).toEqual(['DATA,1931,1076,0,0', 'DATA,1930,1075,1,0']);
    await clock.advance(BRIDGE_MIN_INTERVAL_MS * 3);
    expect(sink.sent).toContain('DATA,1930,1075,1,0'); // 클릭은 사라지지 않는다
  });
