...
describe('§7.5 보기 4 — 4-2-1: 얼굴 마우스 → LCD·서보·버저(f104 원본 그대로)', () => {
  it('좌표는 밀리면 최신 값만, 윙크(클릭 1)는 반드시 한 번 간다', async () => {
    ...
    // 보통 프레임은 최신 값만 남는다(다섯 번 보냈지만 선에는 세 개). 최신 값은 맨 뒤로 옮겨 가므로 윙크가 먼저 나가고,
    // 보드가 마지막에 보는 값이 최신 좌표다(2026-09-25 Phase 4 검토 반영 — 전에는 그 자리에서 바꿔 끼워 순서가 거꾸로 됐다).
    expect(raw).toEqual(['DATA,1920,1080,0,0', 'DATA,1930,1075,1,0', 'DATA,1932,1077,0,0']);
  });
