...
describe('통신 템플릿 파일', () => {
  ...
  it('실습실 예제 id는 templates-…이고 갤러리 태그(사이드카)는 통신 방식까지 적혀 있다', () => {
    ...
    // 가상 BLE(P4-03)·가상 와이파이·MQTT(P4-06)가 들어와 세 템플릿 모두 가상 보드에서 끝까지 돈다(tests/e2e/esp32-comm-blocks.spec.ts가 확인 —
    // 2026-09-24 통합에서 virtual_ok를 적었다. 확인하기 전에는 "모르면 적지 않는다" 갤러리 태그 규약대로 비워 두었다).
    expect(sidecarOf(TEMPLATES.ble)).toMatchObject({ unit: 3, comm: ['ble'], virtualOk: true });
    expect(sidecarOf(TEMPLATES.mqtt)).toMatchObject({ unit: 3, comm: ['wifi', 'mqtt'], virtualOk: true });
    // ESP32 예제 사이드카는 title이 꼭 있어야 한다(tests/unit/lab/esp32-examples.test.ts) — .py 머리말 첫 줄과 같다
    for (const file of Object.values(TEMPLATES)) {
      expect(sidecarOf(file).title, file).toBe(readExampleMeta(fs.readFileSync(path.join(TEMPLATE_DIR, file), 'utf8')).title);
    }
  });
