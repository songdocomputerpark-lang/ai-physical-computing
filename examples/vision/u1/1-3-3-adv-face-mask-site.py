# 1-3-3 심화 실습(사이트판): 얼굴에 마스크 그림 씌우기
...
# ── 바꿔볼 것 3가지 ──
# 1. mask.png 대신 내 그림을 씌워 봐요. 오른쪽 파일 패널의 [파일 넣기]로 투명 배경 PNG를 넣고 15행의 파일 이름을 그 이름으로 바꿔요.
...
# 원본 코드는 이 계산을 파이썬 반복문으로 픽셀마다 했는데, 브라우저 안 파이썬에서는 한 장에 0.5초가 걸려 영상이 뚝뚝 끊겨요.
