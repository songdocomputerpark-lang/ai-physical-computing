...
# 성공 조건 기준 설정
TARGET_Y = 0.3    # 왼손 y 좌표가 이 값보다 작으면 성공으로 판단 @slider 0.05 0.95 0.05 ([사이트판] 조절 막대)
left_wrist_y = 1.0    # [사이트판] 첫 장면에 사람이 없어도 46행에서 비교할 수 있게 처음 값(화면 맨 아래)을 정해 둬요
# 영상 처리 루프 시작
...
# ── 바꿔볼 것 3가지 ──
# 1. 실행한 채로 조절 패널의 TARGET_Y 막대를 0.3에서 0.6으로 옮겨 봐요. Success!가 더 오래 떠요. 0.2까지 줄이면 어떻게 될까요?
# 2. 33행의 LEFT_WRIST를 RIGHT_WRIST로 바꿔 봐요. 재생 입력의 '손 들기' 동작에서는 Success!가 뜨지 않아요.
# 3. 46행의 < 를 > 로 바꿔 봐요. 이번에는 손을 내리고 있을 때 Success!가 떠요.
# ── 왜 이런 결과가 나올까 ──
# 33행은 몸의 점 33개 가운데 15번 점(왼쪽 손목)의 y 좌표를 꺼내요. y는 영상 맨 위가 0, 맨 아래가 1이에요.
# 그래서 손을 번쩍 들수록 y가 작아져요. 46행은 그 값이 TARGET_Y보다 작을 때만 Success!를 써요.
# TARGET_Y를 크게 하면 기준선이 아래로 내려가 손을 조금만 들어도 성공이 되고, 작게 하면 더 높이 들어야 해요.
# 15행은 사람이 안 보이는 장면에서도 46행이 비교할 값이 있게 처음 값 1.0(화면 맨 아래)을 정해 둬요. 이 줄이 없으면 NameError가 나요.
