...
while cv2.getWindowProperty("Face",cv2.WND_PROP_VISIBLE):
    ...
    if results.multi_face_landmarks:
        ...
        left_eye_dist=(int(lm.landmark[158].x*width)-int(lm.landmark[153].x*width))**2+\
            (int(lm.landmark[158].y*height)-int(lm.landmark[153].y*height))**2
        left_eye_norm=(int(lm.landmark[133].x*width)-int(lm.landmark[33].x*width))**2+\
            (int(lm.landmark[133].y*height)-int(lm.landmark[33].y*height))**2   
        left_eye_closed=left_eye_norm*0.05>left_eye_dist
        eye_status="Opend" if not left_eye_closed else "Closed"
