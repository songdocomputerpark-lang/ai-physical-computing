...
# ── 바꿔볼 것 3가지 ──
# 1. cells를 16으로 올려 봐요. 같은 자리를 더 잘게 나눠 보게 되고 숫자가 작아져요. 같은 넓이에 픽셀이 몇 개 들어가는지가 해상도예요.
# 2. x, y를 움직여 밝은 곳과 어두운 곳을 확대해 봐요. 흰 물체는 200이 넘고, 그림자는 50 아래로 내려가요.
# 3. gray 대신 frame[:, :, 2](빨강 채널)를 잘라 확대해 봐요. 빨간 물체는 숫자가 크고 파란 물체는 작아요(V2에서 자세히 봐요).
# ── 왜 이런 결과가 나올까 ──
