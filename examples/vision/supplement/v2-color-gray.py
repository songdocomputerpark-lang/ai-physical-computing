...
# 2. mode를 swap으로 바꿔 봐요. 파란 것은 주황·빨강으로, 빨간 것은 파랑으로 뒤바뀌어요. 숫자는 그대로인데 읽는 순서만 달라진 거예요.
# 3. gray를 ((blue.astype(int) + green + red) // 3).astype("uint8")로 바꿔 봐요. 세 채널을 똑같이 평균해도 회색이 되지만 OpenCV 방식과 조금 달라요.
# ── 왜 이런 결과가 나올까 ──
