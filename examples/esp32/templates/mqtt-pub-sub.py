...
WIFI_PASSWORD = "my-password"  # 와이파이 비밀번호로 바꿔요(다른 사람에게 알려 주지 않아요).
BROKER = "broker.emqx.io"  # 학습용 공개 브로커. 이름·사진 같은 개인정보는 절대 보내지 않아요.
DEVICE = "esp32-01"  # 내 보드 번호(이름·학번은 넣지 않아요)
# 토픽은 "내보드/rx"처럼 짧게 적어요. 우리 반 접두어(무작위 12글자)는 사이트가 앞에 붙여 주고 화면에 보여 줘요.
TOPIC_RX = DEVICE + "/rx"  # 컴퓨터 → 보드(명령)
TOPIC_TX = DEVICE + "/tx"  # 보드 → 컴퓨터(값)
...
# 토픽은 "esp32-01/tx"처럼 짧게 적지만, 실제로 나갈 때는 앞에 우리 반 접두어(무작위 12글자)가 붙어요. 공개 브로커는
# 온 세상이 함께 쓰는 곳이라, 접두어가 없으면 "esp32-01/tx"를 쓰는 다른 학교 메시지와 섞여요.
