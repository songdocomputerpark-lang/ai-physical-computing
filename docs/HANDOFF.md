...
운영자가 **다른 Claude 계정으로 로그인**하거나 **새 대화**를 열고 "사이트 작업 재개"라고 말했을 때, 새 Claude가 헤매지 않고 이어서 일하기 위한 문서다. 마지막 갱신 2026-09-26.

> **모든 Phase 완료(2026-09-26, 판 1.0.0) — 앞으로의 유지보수는 `MAINTENANCE.md`.** 도는 워크플로는 없다. 새 차시·예제·그림·출처 등록·판 올리기·연 1회 점검·오프라인판은 MAINTENANCE.md를 따르고, 남은 일은 `PROGRESS.md`의 "운영자 할 일"(실물·교실 확인, 대응 소스 사본·릴리스)과 "미해결" 표다. 아래 4절의 워크플로 이어받기는 앞으로 큰 기능을 새로 만들 때 쓰는 기록이다.
...
| Phase 6 품질과 유지보수 | `wf_c7485e2c-a5b` | `workflows\scripts\phase6-quality-wf_c7485e2c-a5b.js`(사본 `.cache/resume/phase6-quality-wf_c7485e2c-a5b.js`) | 완료(2026-09-26 — 시나리오 E·검토 반영까지, 판 1.0.0 — **모든 Phase 끝**) |
...
### 끝난 것 — `wf_c7485e2c-a5b` Phase 6 품질과 유지보수 (2026-09-26 03:53 시작 → 같은 날 완료)
...
**P6-08·검토·수정 단계 끝(2026-09-26):** 시나리오 E 통과(push 뒤 51.6초에 새 차시 주소 200) → 적대적 검토 3명(4911~4913 — 완료 기준·사용성·안전, `.cache/phase6-reviews/`)의 28건 → 수정 단계가 반영·전체 검증·영역별 경로 지정 커밋·push·CI 초록·실사이트 확인·오프라인판 1.0.0 zip 확인까지 했다. 커밋 목록·CI 실행 번호·측정값은 PROGRESS "현재 Phase"와 작업 로그, 보고는 `.cache/phase6-notes/final.md`. 이 워크플로는 끝났으니 다시 돌리지 않는다.
...
**모든 Phase가 끝났다(2026-09-26, 판 1.0.0) — 앞으로의 유지보수는 `MAINTENANCE.md`.** 도는 워크플로는 없다. 완료 판정표·프로젝트 전체 완료 판정·남은 운영자 할 일은 PROGRESS "현재 Phase"·"운영자 할 일"에 있다. 판을 올리거나 Phase 완료를 판정하기 전에는 main의 CI "테스트"가 초록이어야 한다(DECISIONS C38). 큰 기능을 새로 만들 때 지금까지 쓴 틀은 **Core 순차 단계 → 병렬 구역(구역마다 고칠 파일을 못 박고 포트 49xx, `PW_BASE_URL`로 각자 시험) → 통합 → 적대적 검토 3명 → 수정·배포**이고, 에이전트는 `effort: 'max'`, 공통 프롬프트에 **파일 내려받기 금지**(외부 파일은 운영자만 받는다)를 넣는다.
