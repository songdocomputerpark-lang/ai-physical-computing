/**
 * 가상 ESP32 보드 모듈의 화면 쪽(PLAN §8.3 P3-01, src/lab/README.md 7절). 파이썬 쪽은 같은 폴더의 machine.py·apc_board.py.
 *
 * 하는 일
 * 1. ESP32 실습실의 io 슬롯(src/components/lab/BoardIo.astro의 [data-board-io])에 보드 그림·부품·핀 표를 그린다(view.ts).
 * 2. 배선: 보드에 붙은 부품(내장 LED·BOOT 버튼) + 지금 예제의 parts(LabExample.parts — P3-02가 채움)를 검사해(parts.ts resolveWiring)
 *    부품을 놓고, [실행] 직전에 'board.wiring'으로 파이썬에 알린다.
 ...
 * 4. 출력: 파이썬의 'board.state' 이벤트(핀 상태 묶음)를 받아 부품 모습과 핀 표를 고친다. [정지]·다시 시작으로 끝나면 부품을 꺼진 모습으로
 *    되돌리고(코드가 스스로 끝나면 마지막 모습 유지 — 실물과 같음), 다음 [실행]의 'reset'에서 새로 그린다.
...
import type { PartInstance, WiringEntry } from './part-types.ts';
...
function mount(context: LabModuleContext): LabModuleHandle | void {
  ...
  const applyWiring = () => {
    ...
    view.setWiring(instances, resolved.problems);
    drives = inputDrives(instances, view.activeIds, PART_DEFINITIONS);
    context.setValue(BOARD_CHANNEL_WIRING, wiringValue(instances));
    context.setValue(BOARD_CHANNEL_INPUTS, inputsValue(drives));
    view.update(snapshot);
  };
