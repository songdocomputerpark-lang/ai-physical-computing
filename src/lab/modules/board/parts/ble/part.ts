...
const definition: PartDefinition = {
  ...
  controls(host, api) {
    ...
    received.style.fontFamily = 'var(--font-mono)';
    ...
    const appNote = element('p', {}, '스마트폰 앱처럼 이진 프레임(FF 02 01 01 …)을 보내요. 4-단원 보드 확인 예제(스마트폰 앱 → RGB LED)에 쓰는 칸이에요.');
    ...
    const previewText = () => {
      ...
      preview.textContent = `보낼 값: ${message}${ending === 'lf' ? '\\n' : ''}`;
    };
    ...
    const render = () => {
      ...
      received.textContent = bleLastReceivedText(state);
