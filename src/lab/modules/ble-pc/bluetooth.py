...
class BLEUARTBridge:
    ...
    def __init__(self, address):
        ...
        if reply.get("error"):
            ...
            # 화면 안내(notices)가 "보드 화면을 열고 [연결]을 눌러요"라고 알려 준다.
            self._closed = True
            raise ConnectionError(str(reply.get("error")))
        self._label = reply.get("label")
        if self._label:
            print("Connected to %s" % self._label)
    ...
    def connected(self):
        ...
        if self._closed:
            return False
        _drain_received()
        return bool(_info().get("connected"))

    def send(self, data):
        """글자(또는 바이트)를 상대에게 보낸다. 원본처럼 끝 문자를 붙이지 않는다."""
        if not self.connected:
            # 원본도 연결 전에는 보내지 않고 이 줄을 찍는다(connection_event.wait가 시간을 넘긴 갈래).
