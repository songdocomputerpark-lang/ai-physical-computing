...
export function noPeerNotice(): string {
  return [
    '블루투스로 이을 보드를 아직 찾지 못했어요.',
    '같은 화면의 가상 보드(또는 ESP32 실습실)에서 보드 코드를 [실행]하고 블루투스 칸의 [연결]을 누르면',
...
function mount(context: LabModuleContext): LabModuleHandle {
  ...
  const openLink = () => {
    ...
    bridge = createBridge(a, {
      ...
      terminator: '',
      onSend: (message: BridgeMessage, line: string) => {
        sent += 1;
