...
function mount(context: LabModuleContext): LabModuleHandle {
    ...
    void (async () => {
      try {
        ...
        if (status.state !== 'open') {
          request.reply({ ok: false, reason: 'closed', error: status.error ?? '시리얼 통로를 열지 못했어요.' });
          return;
        }
  ...
  context.onEvent('serial-pc.tx', (payload) => {
    ...
    // 새 예제용 bridge 모듈은 상태·이벤트를 싣는다(bridge.event → 'event'). pyserial 흉내는 싣지 않는다 — 원본 코드는 모양으로 판정(§7.6).
    const category = (payload as { category?: unknown })?.category;
    link.sendBytes(bytes, {
      baud: typeof baud === 'number' ? baud : 0,
