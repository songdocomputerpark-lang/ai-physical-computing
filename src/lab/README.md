...
- `load`(선택, Phase 6 P6-02 — PROGRESS 미해결 157, DECISIONS C17): 화면 쪽(`index.ts`)을 **쓸 때만** 받는 규칙 `{ group, code, query?, windowEvents?, order? }`(types.ts `LabModuleLoadRule`).
  적지 않으면 지금까지처럼 실습실이 열릴 때 받아요. 적으면 host가 ① 편집칸 코드에 `code` 정규식이 맞을 때(열 때의 코드와 `code` 이벤트 — 예제·공유 링크·자동 저장·블록 모드 생성 코드 모두)
...
- `install()`이 예외를 내면 그 흉내 모듈만 건너뛰고 나머지는 설치해요(`apc_shims.install_available()`). 실패는 `apc_shims.last_failures()`에 모이고 워커가 "사이트 흉내 모듈(…)을 준비하지 못했어요(…). 실행은 이어서 해요." 한 줄로 알려요 — 단, 패키지를 받는 중이면(영상처리 실습실이 OpenCV를 미리 받는 동안 [실행]) 풀리는 중인 패키지라 알리지 않고 다음 실행에서 다시 설치해요(2026-09-25 Phase 5 검토 — 두 번째 실행에서 "Traceback" 알림이 뜨던 것).
...
HTML·CSS만 그리고 동작은 index.ts가 `data-<id>-*` 표시로 찾아 잇어요. LabShell이 `<section data-lab-module-panel="<id>" hidden>` 안에 그려요 — `placement: 'panel'`은 조절 패널 아래(오른쪽 열), `'wide'`는 입력·출력 줄과 콘솔 사이 전체 폭(가상 데스크톱처럼 넓은 화면용). 실습실 부품 클래스(`lab-button`, `lab__select`)와 디자인 토큰(`src/styles/`)을 그대로 써요. 접근성: 조작 요소에 label, 살아 있는 글에 `aria-live`, 키보드로 닿게. 넘칠 수 있는 칸(기록·코드·표를 감싼 칸)에는 `data-scroll-focus="칸 이름"`을 붙여요 — 넘치는 동안만 키보드로 들어와 밀어 볼 수 있게 돼요(`src/components/common/scroll-focus.ts`, 스크립트가 만든 칸은 `element.dataset.scrollFocus = '…'`). 역할 없는 `<pre>`·`<code>`·`<div>`에 aria-label을 달지 않아요(화면 낭독기가 읽지 않음 — role을 주거나 보이는 이름표를 앞에, 2026-09-26 Phase 6 접근성 — DECISIONS C19).
...
| E 오프라인 배포판(**2차 — Build2**) | 4905 | P6-07(`npm run build:offline` — §5.6, zip 안 PowerShell 작은 웹 서버(`serve.ps1`)·`시작하기.bat`, 인터넷 없이 첫 실습·가상 보드·같은 컴퓨터 탭 통신) | `scripts/build-offline.mjs`, `scripts/lib/offline-*`, `scripts/offline/**`, `scripts/fetch-pyodide-fallback.mjs`, `scripts/build-sw.mjs`, `src/sw/**`, `src/lab/loader/{pyodide-files,constants,sw-client}.ts`, `src/lab/runtime/config.ts`, `tests/e2e/offline*.spec.ts`, `tests/unit/offline/**`, `tests/unit/loading/{build-sw,pyodide-files,sw-client}.test.ts` | `npm run build:offline` → `npm run test:offline`(zip 풀기 → 서버 → `tests/e2e/offline.spec.ts`) |
...
| 가상 시각 알람(2026-09-26) | `apc_board.register_wake_hook(hook)` — `hook(now_ns)`가 할 일을 하고 다음에 깨울 가상 시각(나노초, 없으면 None)을 돌려준다. `wait_ns`·`wait_ns_async`가 Timer처럼 그 시각에 sleep을 끊는다(MP3 곡 끝 — 미해결 177). 지난 시각은 알람이 아니고, 양보 금지 | — | `tests/unit/board-uart/pyodide-mp3.test.ts`(`wake_hook_contract`) |
