...
const RULES: readonly CompatRule[] = [
  ...
  },
  {
    ...
    on: 'bare',
    /*
     * 대괄호 안에 콜론 둘. 중괄호(사전)와 쉼표는 문자 묶음에서 빼서 `colors = [{'r':255,'g':0,'b':0}]`처럼 자르기가 없는
     * 흔한 줄을 잘못 지적하지 않게 한다(2026-09-18 검토 반영 — MP3·네오픽셀 수업에서 트랙·색 목록을 이렇게 적는다.
     * 글자 안을 비운 줄로 검사하므로 그 줄은 `[{'':255,'':0,'':0}]`이 되어 옛 규칙에 걸렸다).
     */
    pattern: /\[[^[\]{},\n]*:[^[\]{},\n]*:[^[\]{},\n]*\]/u,
