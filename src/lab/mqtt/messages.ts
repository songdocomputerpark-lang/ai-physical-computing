...
export const mqttText = {
  ...
  /**
   * [연결] 단추 바로 아래에 **늘** 보이는 짧은 경고(§7.4 "연결 버튼 옆에 늘 표시" — 2026-09-25 Phase 4 검토 반영:
   * 휴대폰에서는 칸 맨 위 경고가 [연결]과 800px 떨어져 연결하는 순간 보이지 않았다).
   */
  connectWarning(): string {
    return '공개 중계 서버: 누구나 보고 보낼 수 있어요. 개인정보를 보내지 말고, 움직이는 장치(레이저·모터)는 잇지 않아요.';
  },
  /**
   * [실제 보드] 탭을 골랐을 때 칸 맨 위 경고(2026-09-25 Phase 4 검토 반영 — 전에는 통로 값만 보고 "이 컴퓨터 밖으로 나가지 않아요"를 보였는데,
   * 실제 보드는 보드의 와이파이로 공개 중계 서버에 직접 붙는다).
   */
  realBoardWarning(): string {
    return (
      '실제 보드는 코드에 적은 중계 서버(broker.emqx.io 같은 공개 중계 서버)에 보드의 와이파이로 직접 붙어요 — 아래 통로·[연결]은 가상 보드에만 쓰여요. ' +
      '공개 중계 서버는 누구나 보고 보낼 수 있어요. 개인정보를 보내지 말고, 토픽 앞에 통신 접두어를 적고, 받은 명령으로는 LED·LCD 같은 표시 장치만 움직여요.'
    );
  },
  /** [실제 보드] 탭일 때 [연결] 아래 짧은 줄 */
  realBoardConnectNote(): string {
    return '지금은 [실제 보드] 탭이에요. 실제 보드는 이 [연결]을 쓰지 않고 코드에 적은 중계 서버에 직접 붙어요(누구나 보고 보낼 수 있어요).';
  },
  /** 학생 코드가 retain=True로 보냈을 때 한 번(PLAN §7.4 retain 끔) */
  retainIgnored(): string {
    return (
      'retain=True는 쓰지 않아요 — 공개 중계 서버에 값이 남아 나중에 그 토픽을 받는 누구에게나 전해지기 때문이에요. ' +
      '가상 보드는 retain을 끄고 보냈어요. 실제 보드에 올릴 코드에서도 retain=True를 빼요.'
    );
  },
  /** 코드의 토픽이 다른 통신 접두어로 시작할 때 한 번 */
  foreignPrefix(codePrefix: string, prefix: string): string {
    return (
      `코드의 토픽 접두어(${codePrefix})가 이 칸의 통신 접두어(${prefix})와 달라요. 가상 보드는 이 칸의 접두어를 앞에 한 번 더 붙여서 보내요 — ` +
      '같게 하려면 [친구 접두어]에 코드의 접두어를 넣고 [맞추기]를 누르거나, [코드에 접두어 적기]를 눌러요.'
    );
  },
  /** [코드에 접두어 적기]가 PREFIX 줄에 접두어를 적었을 때 */
  prefixWritten(name: string, prefix: string): string {
    return `코드의 ${name} 줄에 통신 접두어 ${prefix}를 적었어요. 이제 가상 보드와 실제 보드가 같은 토픽으로 주고받아요.`;
  },
  /** PREFIX 줄을 찾지 못했을 때 */
  prefixLineMissing(prefix: string): string {
    return `코드에서 PREFIX = "" 같은 줄을 찾지 못했어요. 실제 보드에 올릴 코드는 토픽 앞에 "${prefix}/"를 직접 적어요(예: "${prefix}/esp32-01/rx").`;
  },
  /** 블록 모드라 편집칸을 고칠 수 없을 때 */
  prefixInBlocks(): string {
    return '블록 모드에서는 편집칸을 고칠 수 없어요. [코드로 바꾸기]를 누른 뒤 [코드에 접두어 적기]를 다시 눌러요.';
  },
  /** 연결 성공 */
  ...
  prefixAdded(prefix: string): string {
    return (
      ...
      '실제 보드는 스스로 붙이지 못하니, 실물에 올릴 코드에는 토픽 앞에 이 접두어를 직접 적어요.'
    );
  },
  /** 장치 이름 규칙 */
