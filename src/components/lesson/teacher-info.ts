...
import { STANDARDS_PENDING_TEXT, pagesText } from './lesson-data.ts';
...
export interface TeacherInfoInput {
  readonly data: Pick<LessonData, 'kind' | 'pages' | 'standards' | 'handouts'>;
...
function sourceText(source: LessonSource | undefined, kind: LessonData['kind'], pages: string | undefined): string | undefined {
  ...
  switch (source) {
    ...
    case 'code-only':
      return `${NO_MANUSCRIPT_TEXT}이에요. 교과서 원고가 아직 없어서 예제 코드와 주석${where ? `(${where})` : ''}, 수업 교안, 대단원 마무리 문항을 바탕으로 썼어요. 원고를 받으면 원고로 바꿔요.`;
    case 'supplement':
      return '보충 차시: 교과서에 없는 내용을 사이트가 새로 썼어요.';
    default:
      return undefined;
  }
}
...
export function buildTeacherInfo({ data, source, handoutExists }: TeacherInfoInput): TeacherInfo {
  return {
    source: sourceText(source, data.kind, data.pages),
    ...
    originals:
      '원본 내려받기는 없어요. 원고 PDF, 수업 교안, 코드 압축 파일 같은 원본은 사이트에 올리지 않아요. 원본에 사람 얼굴, 컴퓨터 속 파일 경로, 기기 주소 같은 개인정보가 섞여 있을 수 있어서예요. 원고 내용은 이 페이지에 옮겼고, 예제 코드는 실습실의 [.py 내려받기]와 예제 갤러리에서 받을 수 있어요.',
    originalsLinks: [
